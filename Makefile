...
APP_NAME = frog-finance-api
APP_NAME := $(APP_NAME)

.PHONY: build
# Build the container image - Dvelopment
build-dev:
	docker build -t ${APP_NAME} --target development-build-stage -f Dockerfile .
...
run:
	docker-compose --env-file ./.env.development.local -p ff up -d --build api
